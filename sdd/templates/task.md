...
|---|---|---|
| `navigator_session/path/to/new_file.py` | CREATE | Main implementation |
| `tests/unit/test_new_file.py` | CREATE | Unit tests |
| `navigator_session/path/to/existing.py` | MODIFY | Add import / register component |
...
```python
from navigator_session.module import ClassName  # verified: navigator_session/module/__init__.py:NN
```
...
```python
# navigator_session/path/to/file.py:NN
...
- ~~`navigator_session.module.NonExistentThing`~~ — does not exist
...
# e.g. copy this structure from navigator_session/storages/abstract.py
...
- `navigator_session/path/reference1.py` — pattern to follow
- `navigator_session/path/reference2.py` — integration point
...
### `navigator_session/path/to/new_file.py` (CREATE)
```python
"""<module docstring>."""
...
### `navigator_session/path/to/existing.py` (MODIFY)
```python
# occurrences: <N> (verified: grep -c '<verbatim anchor line>' navigator_session/path/to/existing.py)
# AFTER — insert below `<verbatim anchor line>` (verified: navigator_session/path/to/existing.py:NN)
<new lines>
```
...
- [ ] No linting errors: `ruff check navigator_session/<path>`
- [ ] Imports work: `from navigator_session.<module> import <Component>`
- [ ] Criterion N
...
import pytest
from navigator_session.<module> import <Component>
...
4. **Update status** in the per-spec index at `sdd/tasks/index/<feature-slug>.json` → `"in-progress"` with your session ID
